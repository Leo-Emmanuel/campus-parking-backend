...
router.get('/reports/combined/pdf', async (req, res) => {
  try {
    ...
    const [users, events, bookings] = await Promise.all([
      ...
      Booking.find({
        date: { $gte: start, $lte: end }
      }).populate('userId', 'name email').populate('zoneId', 'name type')
        .select('userId zoneId zoneName date duration vehicleNumber status').sort({ date: -1 }).lean()
    ]);
    ...
    bookings.slice(0, 30).forEach((booking) => { // Limit to first 30 bookings
      ...
      const rowData = [
        ...
        booking.zoneName || booking.zoneId?.name || 'N/A',
