...
async function getZoneAvailability(zoneId, session = null) {
  ...
  return {
    ...
    name: zone.name,
    total: zone.total,
    available: Math.max(0, zone.total - activeBookings),
    type: zone.type,
  };
}
...
const userSchema = new mongoose.Schema({
  ...
  role: { type: String, enum: ['student', 'staff', 'admin', 'visitor'], default: 'student' },
...
const zoneSchema = new mongoose.Schema({
  ...
  type: { type: String, enum: ['student', 'staff', 'general', 'visitor'], required: true },
  location: String,
  createdAt: { type: Date, default: Date.now },
});
...
const eventSchema = new mongoose.Schema({
  name: { type: String, required: true },
  date: { type: Date, required: true },
  allocatedSlots: { type: Number, required: true, min: 0 },
  zone: String,
  description: String,
  createdAt: { type: Date, default: Date.now },
});
...
app.get('/api/zones/:zoneId/availability', async (req, res) => {
  try {
    ...
    const available = Math.max(0, zone.total - activeBookings);
...
app.post('/api/bookings', authenticateToken, bookingValidation, handleValidationErrors, async (req, res) => {
  ...
  try {
    ...
    const newAvailable = Math.max(0, zone.total - (activeBookings + 1));
...
async function seedDatabase() {
  try {
    ...
    const zones = [
      { name: 'Zone A - Main Campus', total: 50, type: 'student', location: 'Building A' },
      { name: 'Zone B - Faculty Block', total: 30, type: 'staff', location: 'Building B' },
      { name: 'Zone C - Library', total: 40, type: 'general', location: 'Library' },
      { name: 'Zone D - Visitor Parking', total: 20, type: 'visitor', location: 'Main Gate' },
    ];
